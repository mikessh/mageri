...
public class DummySAssembler extends Assembler<SConsensus, SomewhatRawReadProperlyWrapped> {
    public DummySAssembler() {
        storeConsensuses = false;
    }

    @Override
    public SConsensus assemble(SomewhatRawReadProperlyWrapped mig) {
        readsTotal.incrementAndGet();
        migsTotal.incrementAndGet();
        migsAssembled.incrementAndGet();
        readsAssembled.incrementAndGet();

        return new SConsensus(mig.getSample(), mig.getUmi(), mig.getReadContainer().getFirst().toNucleotideSQPair(),
                new HashSet<Integer>(), 1, 1);
    }
