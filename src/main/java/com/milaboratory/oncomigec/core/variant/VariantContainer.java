...
public class VariantContainer implements Serializable {
    ...
    private void init() {
        ...
        for (int i = 0; i < mutationsAndCoverage.referenceLength(); i++) {
            int sumAtPosMig = 0, majorMigCount, maxMigCount = Integer.MIN_VALUE, minorMigCount;
            long sumAtPosRead = 0, majorReadCount, minorReadCount, accumulatedReadCount;
            ...
            for (byte from = 0; from < 4; from++) {
                ...
                maxMigCount = Math.max(majorMigCount, maxMigCount);
            ...
            if (sumAtPosMig > 0) {
                for (byte from = 0; from < 4; from++) {
                    ...
                    for (byte to = 0; to < 4; to++) {
                        ...
                        accumulatedReadCount = mutationsAndCoverage.getGainedNucleotideReadCount(i, to);
                        ...
                        if (from != to) {
                            ...
                            innerMatrixRead[from][to] += minorReadCount * (double) majorReadCount / (double) sumAtPosRead;
                                ...
                                majorMigCount < maxMigCount) { // store only non-dominant variants
    ...
    public double getBgFreqMig(byte from, byte to) {
        return getBgFreqMig(from, to, true);
    }
    ...
    public double getBgFreqRead(byte from, byte to) {
        return getBgFreqRead(from, to, true);
    }
